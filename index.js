...
function stringToNumber(str) {
  return Number(str); // Використовуємо Number() для перетворення рядка в число
...
function isAlmostSame(num1, num2) {
  return num1 - num2 <= Number.EPSILON || num2 - num1 <= Number.EPSILON; // Визначаємо, чи є різниця між двома числами меншою або рівною EPSILON
}
...
function isSafeInteger(num) {
  return num <= Number.MAX_SAFE_INTEGER && num >= Number.MIN_SAFE_INTEGER; // Перевіряємо, чи є число меншим або рівним MAX_SAFE_INTEGER і більшим або рівним MIN_SAFE_INTEGER
}
...
function isUnsafeInteger(num) {
  return !(num <= Number.MAX_SAFE_INTEGER && num >= Number.MIN_SAFE_INTEGER); // Використовуємо логічну оператор НЕ (!), щоб отримати протилежну відповідь від функції isSafeInteger()
}
...
function isTooLarge(num) {
  return num > Number.MAX_VALUE; // Перевіряємо, чи є число більшим за MAX_VALUE
}
...
function isAlmostZero(num) {
  return num > 0 && num < Number.MIN_VALUE; // Перевіряємо, чи є число більше 0, але все ще менше за Number.MIN_VALUE
}
...
function checkIsInteger(num) {
  return Number.isInteger(num); // Використовуємо вбудовану функцію Number.isInteger(), щоб перевірити, чи є значення цілим числом
}
...
function checkIsSafeInteger(num) {
  return Number.isSafeInteger(num); // Використовуємо вбудовану функцію Number.isSafeInteger(), щоб перевірити, чи є значення безпечним цілим числом
}
...
function convertToExponential(num) {
  return num.toExponential(); // Використовуємо метод toExponential(), щоб конвертувати число в рядок з експоненційним представленням
}
...
function convertToFixed(num, precision) {
  return num.toFixed(precision); // Використовуємо метод toFixed(), щоб конвертувати число в рядок з фіксованою кількістю знаків після коми
}
...
function convertToString(num) {
  return num.toString(); // Використовуємо метод toString(), щоб конвертувати число в рядок
}
...
function convertToPrecision(num, precision) {
  return num.toPrecision(precision); // Використовуємо метод toPrecision(), щоб округлити число до вказаної довжини
}
